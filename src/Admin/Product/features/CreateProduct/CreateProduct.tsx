...
const CreateProduct = () => {
  const {
    ...
    actions: { onChangeFormInput, onChangeImage, handleCreateProduct, resetForm },
  } = useCreateProduct();
    ...
    <>
      <Box h="calc(100vh - 3rem)" p={6}>
        <Flex gap={4} alignItems="center">
          ...
          <IconButton colorScheme="blue" aria-label="reset form" icon={<RepeatIcon />} onClick={handleResetForm} />
        </Flex>
        <Grid h="100%" templateColumns="repeat(2, 1fr)">
          {/* <CreateProductForm /> */}
          <form onSubmit={handleSubmit}>
            <Flex gap={8} mt={4} flexDirection="column">
              <FormControl isInvalid={title.value.length > 0 && title.value.length < 3}>
                <FormLabel>Title</FormLabel>
                <Input width="auto" placeholder="Title" value={title.value} onChange={handleTitleChange} />
                <FormErrorMessage>The title must be 3 or more characters long.</FormErrorMessage>
              </FormControl>
              <FormControl>
          ...
          <Flex justifyContent="center">
            <Box>{image && <Image src={image.src} alt="New product image" />}</Box>
          </Flex>
        </Grid>
      </Box>
      <Modal isCentered isOpen={loading} onClose={() => {}} closeOnOverlayClick={false}>
